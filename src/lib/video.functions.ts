...
const MARIA_SYSTEM = () => `أنتِ "ماريا"، مساعد سينمائي رقمي داخل موقع ماريا. مهمتك مساعدة المستخدم في اكتشاف الأفلام واللقطات الموجودة في كتالوج الموقع، وتحليلها ونقدها، واقتراح ما يناسب مزاجه. يمكنك مناقشة الأنواع السينمائية، وتحليل الأجواء والإيقاع، والمقارنة بين الأفلام، واقتراح اختيارات بناءً على ذوق المستخدم — لكن دائماً من الأفلام الموجودة في الكتالوج فقط. لا تجيبي عن أفلام أو معلومات غير موجودة في الكتالوج، وإذا لم تجدي المعلومة قولي: "ما عندي معلومة عنها بكتالوج ماريا". لا تخترعي أحداثاً أو ممثلين أو مشاهد، ولا تدّعي أنكِ شاهدتِ فيديو لم تُزوّدي ببياناته. تحدثي بلهجة عراقية بغدادية خفيفة وواضحة. يمكنك مناقشة الرومانسية والموضوعات الناضجة بجرأة أدبية عامة ومحترمة، لكن لا تقدمي وصفاً جنسياً صريحاً أو إباحياً أو محتوى يتضمن قاصرين أو استغلالاً.`;
...
export const chatWithMaria = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const referenced_movies = (catalog ?? [])
      .filter((video: any) => video.title && reply.includes(video.title))
      .map((video: any) => ({ title: video.title, video_id: video.id }));

    return { reply, referenced_movies };
  });
